...
class PersistencyMatrix:
    """ A class that remembers repeating patterns in vectors.
    """

    def __init__(self, X):
        """
        Initialize Persistent Matrix with 0 of dimension N * N where N is the number of features in X.
        The number of features are assumed to be number of columns.

        :param X: a np matrix of (samples, features) shape.
        """
        self.N_in = X.shape[1]
        self.persistency_matrix = np.zeros((self.N_in, self.N_in))



    def fit(self, X):
        """
        Iterates through samples to train PM.
        :param X: a np matrix of (samples, features) shape.
        :return:
        """
        self.samples_count = X.shape[0]
        for sample_count in range(self.samples_count):
            for bit_j in range(self.N_in):
                for bit_i in range(self.N_in):
                    # If they are different decrease PM value, else increase.
                    self.persistency_matrix[bit_i, bit_j] += np.tanh(np.abs(X[sample_count, bit_i] - X[sample_count, bit_j]))


    def show(self):
        """
        :return: Just plots the persistency matrxi
        """
        plt.imshow(self.persistency_matrix, cmap='gray', interpolation='none')
        plt.title('Persistency Matrix')
        plt.show()
...
if __name__ == '__main__':
    ...
    X[0:160, 4:8] = np.ones((160,4))
    pmX = PersistencyMatrix(X)
    pmX.fit(X)
    pmX.show()
