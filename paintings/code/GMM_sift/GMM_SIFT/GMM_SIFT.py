...
class gmm_image_sift():
    ...
    def fit(self, order=None):
        ...
        kp, self.sift_des = get_sift_des(self.image)
        if order == None:
    ...
    def score(self, X):
        """Given an object of class gmm_image_sift X find the sum log prob with
        current object
        :X: object of class gmm_image_sift

        Returns:
            the fit score of current model with input X image
        """
        kp, sift_des = get_sift_des(X.image)
...
def model_order_selction(X):
    """
    Given an gmm_image_sift X, reshape it and fit a GMM for it, check it's bic. Pick
...
for genre_i in classes:
    ...
    for image_i in image_collection:
        curr_gmm_image_sift = gmm_image_sift(image_i)
        # set the label for current image
        curr_gmm_image_sift.label = classes.index(genre_i)
        c_image_collection.append(curr_gmm_image_sift)
